...
class execution_timer
{
  protected:
    ...
    void write_report(std::ostream& s)
    {
      ...
      for (std::map<std::string, timing>::const_iterator i = m_timings.begin(); i != m_timings.end(); ++i)
      {
        ...
        {
          throw std::runtime_error("Start of " + i->first + " occurred after finish");
        }
  ...
  public:
    ...
    void start(std::string const& timing_name)
    {
      ...
      {
        throw std::runtime_error("Starting already known timing; causes unreliable results");
      }
      timing t;
      t.start = clock();
    ...
    void finish(std::string const& timing_name)
    {
      if (m_timings.find(timing_name) == m_timings.end())
      {
        throw std::runtime_error("Finishing timing that was not started");
      }
